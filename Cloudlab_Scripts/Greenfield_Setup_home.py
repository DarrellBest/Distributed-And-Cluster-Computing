...
for i in range(params.workerCount + 2):
	...
	if i == 1:
		...
		node.addService(rspec.Execute(shell="/bin/sh",
								command="echo '/users *(rw,sync,no_root_squash)' | sudo tee -a /etc/exports"))
		node.addService(rspec.Execute(shell="/bin/sh",
	...
	# install nfs client
		...
		#node.addService(rspec.Execute(shell="/bin/sh",
		#						command="sudo mkdir -p /nfs/users"))
		node.addService(rspec.Execute(shell="/bin/sh",
		...
		#node.addService(rspec.Execute(shell="/bin/sh",
		#						command="echo '192.168.1.2:/home /home nfs auto,nofail,noatime,nolock,intr,tcp,actimeo=1800 0 0' | sudo tee -a /etc/exports"))
		#node.addService(rspec.Execute(shell="/bin/sh",
		#						command="echo '192.168.1.2:/packages /nfs/packages nfs auto,nofail,noatime,nolock,intr,tcp,actimeo=1800 0 0' | sudo tee -a /etc/fstab"))
